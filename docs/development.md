...
To develop bitcore-node-vpub:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-vpub.git
...
We now will setup symlinks in `bitcore-node-vpub` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-vpub
ln -s ~/bitcoind-rpc-vpub
```
...
cd devnode
mkdir node_modules
touch bitcore-node-vpub.json
touch package.json
```

Edit `bitcore-node-vpub.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-vpub/bin/bitcore-node-vpub start
```