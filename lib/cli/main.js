...
var vpubcorenode = require('..');
var utils = require('../utils');

function main(servicesPath, additionalServices) {
  /* jshint maxstatements: 100 */

  var version = vpubcorenode.version;
  var create = vpubcorenode.scaffold.create;
  var add = vpubcorenode.scaffold.add;
  var start = vpubcorenode.scaffold.start;
  var remove = vpubcorenode.scaffold.remove;
  var callMethod = vpubcorenode.scaffold.callMethod;
  var findConfig = vpubcorenode.scaffold.findConfig;
  var defaultConfig = vpubcorenode.scaffold.defaultConfig;
  ...
  program
    ...
    .option('-d, --daemon', 'Make vpubcore-node a daemon (running in the background)')
    .action(function(cmd){
      if (cmd.config) {
  ...
  program
    ...
    .action(function(services){
      ...
      if (!configInfo) {
        throw new Error('Could not find configuration, see `vpubcore-node create --help`');
      }
      var opts = {
      ...
      console.log();
      console.log('    $ vpubcore-node add wallet-service');
      console.log('    $ vpubcore-node add insight-api');
      console.log();
    });
  ...
  program
    ...
    .action(function(services){
      ...
      console.log('  Examples:');
      console.log();
      console.log('    $ vpubcore-node remove wallet-service');
      console.log('    $ vpubcore-node remove insight-api');
      console.log();
    });
