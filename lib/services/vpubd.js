...
var Proposal = vpubcore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to vpubd in Node.js. Manages starting and
 * stopping vpubd as a child process for application support, as well as connecting
 * to multiple vpubd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Vpub(options) {
  if (!(this instanceof Vpub)) {
    return new Vpub(options);
  }
...
util.inherits(Vpub, Service);

Vpub.dependencies = [];

Vpub.DEFAULT_MAX_TXIDS = 1000;
Vpub.DEFAULT_MAX_HISTORY = 50;
Vpub.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Vpub.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Vpub.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Vpub.DEFAULT_SPAWN_RESTART_TIME = 5000;
Vpub.DEFAULT_SPAWN_STOP_TIME = 10000;
Vpub.DEFAULT_TRY_ALL_INTERVAL = 1000;
Vpub.DEFAULT_REINDEX_INTERVAL = 10000;
Vpub.DEFAULT_START_RETRY_INTERVAL = 5000;
Vpub.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Vpub.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Vpub.DEFAULT_INSTANTSEND_FEE = 10000;
Vpub.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Vpub.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Vpub.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Vpub.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Vpub.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Vpub.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Vpub.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Vpub.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Vpub.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Vpub.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Vpub.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Vpub.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Vpub.prototype._initCaches = function() {
...
Vpub.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Vpub.prototype.getAPIMethods = function() {
  var methods = [
...
Vpub.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'vpubd/' + name, 'total:', this.subscriptions[name].length);
};

Vpub.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'vpubd/' + name, 'total:', this.subscriptions[name].length);
};

Vpub.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (vpubcore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'vpubd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Vpub.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'vpubd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Vpub.prototype.unsubscribeAddressAll = function(emitter) {
...
Vpub.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Vpub.DEFAULT_CONFIG_SETTINGS;
...
Vpub.prototype._parseVpubConf = function(configPath) {
  var options = {};
...
Vpub.prototype._expandRelativeDatadir = function() {
...
Vpub.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in vpubd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in vpubd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in vpubd config options');
  ...
  _.extend(this.spawn.config, this._parseVpubConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseVpubConf(networkConfigPath));
  }
...
Vpub.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of vpubcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of vpubcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of vpubcore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in vpub.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that vpubd is undergoing a reindex. ' +
             ...
             'of vpubcore-node services will start.');
    node._reindex = true;
  }
};

Vpub.prototype._resetCaches = function() {
 ...
 * @private
 */
Vpub.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Vpub.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Vpub.prototype._initChain = function(callback) {
  var self = this;
...
Vpub.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === vpubcore.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

Vpub.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === vpubcore.Networks.testnet) {
    networkPath = 'testnet3/vpub.conf';
...
Vpub.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === vpubcore.Networks.testnet) {
...
Vpub.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('vpubd/hashblock', message.toString('hex'));
    }
  }

};

Vpub.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Vpub.prototype._updateTip = function(node, message) {
  var self = this;
...
Vpub.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Vpub.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('vpubd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Vpub.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = vpubcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('vpubd/rawtransaction', message.toString('hex'));
    }

    var tx = vpubcore.Transaction();
    tx.fromString(message);
    var txid = vpubcore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Vpub.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = vpubcore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('vpubd/transactionlock', message.toString('hex'));
    }

  }
};

Vpub.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Vpub.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Vpub.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Vpub.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Vpub.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  }, node._reindexWait || Vpub.DEFAULT_REINDEX_INTERVAL);

};

Vpub.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Vpub.prototype._stopSpawnedVpub = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
...
Vpub.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedVpub(function(err) {
    if (err) {
      return callback(err);
    }
...
Vpub.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
 ...
 */
Vpub.prototype.start = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Vpub.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Vpub.prototype.syncPercentage = function(callback) {
  var self = this;
...
Vpub.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, vpubcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Vpub.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Vpub.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = vpubcore.Script.fromAddress(delta.address);
    return {
...
Vpub.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Vpub.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Vpub.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in vpubcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Vpub.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Vpub.prototype._getConfirmationsDetail = function(transaction) {
...
Vpub.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Vpub.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Vpub.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A vpub transaction id
 * @param {Function} callback
 */
Vpub.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Vpub.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof vpubcore.Address) {
...
Vpub.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Vpub.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Vpub.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Vpub.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Vpub.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Vpub.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Vpubcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Vpub.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = vpubcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Vpub.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Vpub.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Vpub.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Vpub.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Vpub.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Vpub.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Vpub.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Vpubcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Vpub.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Vpub.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addTxlockToTx(tx,result) {
    tx.txlock = false;
    if (tx.feeSatoshis >= Vpub.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.0001 check for txlock
      var rawTx = new vpubcore.Transaction(result.hex);
      var hash = vpubcore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
      if (self.node.services.vpubd.zmqKnownTransactionLocks.get(id)) {
 ...
 * @param callback
 */
Vpub.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Vpub.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Vpub.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Vpub.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Vpub.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Vpub.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Vpub.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Vpub.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Vpub.prototype.govCount = function (callback) {
  var self = this;
...
Vpub.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Vpub.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Vpub.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Vpub.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Vpub.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Vpub.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Vpub.prototype.getInfo = function(callback) {
  var self = this;
...
Vpub.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Vpub.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('vpubd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('vpubd process did not exit'));
      }
