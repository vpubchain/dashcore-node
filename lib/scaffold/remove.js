...
/**
 * Will remove a service from vpubcore-node.json
...
/**
 * Will remove the Node.js service and from the vpubcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The vpubcore-node configuration directory
...
function remove(options, done) {
  ...
  var vpubcoreConfigPath = path.resolve(configPath, 'vpubcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(vpubcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a vpubcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to vpubcore-node.json
        removeConfig(vpubcoreConfigPath, service, next);
      });
    }, done
  );
}
