...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for vpubd is necessary for this upgrade with the "reindex=1" vpub.conf option. \n' +
                 'There are changes necessary in both vpub.conf and vpubcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/vpubevo/vpubcore-node/blob/vpubd/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed vpubcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './vpubcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in vpubcore-node services directory
    ...
    if (servicePackage.vpubcoreNode) {
      serviceModule = service.name + '/' + servicePackage.vpubcoreNode;
    }
 ...
 * @param {Node} node - The Vpubcore Node instance
 */
