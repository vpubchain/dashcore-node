...
/**
 * Will return the path and default vpubcore-node configuration. It will search for the
 * configuration file in the "~/.vpubcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.vpubcore');
  var defaultConfigFile = path.resolve(defaultPath, './vpubcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['vpubd', 'web'];
