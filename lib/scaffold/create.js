...
var BASE_PACKAGE = {
  description: 'A full Vpub node build with Vpubcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    '@vpubevo/vpubcore-lib': '^' + vpubcore.version,
    '@vpubevo/vpubcore-node': version
  }
};

/**
 * Will create a directory and vpub.conf file for Vpub.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createVpubDirectory(datadir, done) {
...
/**
 * Will create a base Vpubcore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The vpub database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/vpubcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Vpubcore Node directory, configuration file,
 * vpub configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the vpubcore node configuration directory
 * @param {String} options.datadir - The path to the vpub datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the vpubcore-node directory and configuration
    ...
    function(next) {
      // Setup the Vpub directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createVpubDirectory(absDataDir, next);
      } else {
        next();
      }
    },
